...
static void save_dynamic_sram(uint16_t *offset) {
    ...
    while (remaining != 0u) {
        ...
        memcpy(
            sram_copy_buf,
            SRAM_BASE + (src_offset - Z_DYNAMIC_WRAM_SIZE),
            chunk
        );
...
static void restore_dynamic_sram(uint16_t *offset) {
    ...
    while (remaining != 0u) {
        ...
        SWITCH_RAM(Z_DYNAMIC_SRAM_BANK);
        memcpy(
            SRAM_BASE + (dst_offset - Z_DYNAMIC_WRAM_SIZE),
            sram_copy_buf,
            chunk
        );
...
void z_mem_init(void) {
    ...
    if (z_static_base < Z_DYNAMIC_WRAM_SIZE ||
    ...
    for (i = Z_DYNAMIC_WRAM_SIZE; i < z_static_base; i++) {
        SRAM_BASE[i - Z_DYNAMIC_WRAM_SIZE] =
            rom_read_byte((uint32_t)i);
    }
}
