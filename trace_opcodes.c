...
uint8_t z_save_state(void) {
    printf("[CALL z_save_state]\n");
    return 1;
}
uint8_t z_restore_state(void) {
    printf("[CALL z_restore_state]\n");
    return 0; // return 0 = failed restore
}
...
char workboy_get_char(void) { return 0; }
...
int main(void) {
    ...
    printf("Tracing opcodes during boot...\n");
    while (1) {
        uint32_t pc = z_machine_pc;
        uint8_t op = z_read_byte(pc);
        if (op == 0xB5 || op == 0xB6) {
            printf("PC=0x%04X: Special Opcode 0x%02X (SAVE/RESTORE)\n", pc, op);
        }
        if (op == 0xE4) {
            printf("PC=0x%04X: SREAD reached\n", pc);
            break;
        }
