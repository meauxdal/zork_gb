...
static uint8_t mock_joypad_val = 0;
uint8_t joypad(void) { return mock_joypad_val; }
...
char workboy_get_char(void) {
    static int polls_for_char = 0;
    polls_for_char++;
    if (polls_for_char < 3) return 0;
    polls_for_char = 0;

    if (sread_count >= 3) {
        printf("\n=== FINISHED 3 SREAD INPUTS ===\n");
        exit(0);
    }

    const char *cur_inp = inputs[sread_count];
    if (char_idx < strlen(cur_inp)) {
        char ch = cur_inp[char_idx++];
        return ch;
    } else {
        char_idx = 0;
        sread_count++;
        printf("\n [Submitting input #%d: '%s']\n", sread_count, cur_inp);
        return '\n';
    }
}
...
int main(void) {
    ...
    z_write_byte(0x20u, 18u);
    z_write_byte(0x21u, 20u);

    printf("=== RUNNING GAME WITH 3 EMPTY INPUTS ===\n");
    while (1) {
